...
LEADER_CSV = 'circulo_log_1778102694.csv' \
''   # ← CAMBIAR AQUÍ
...
def plot_results(buf):
    ...
    Genera las mismas 7 figuras que el script MATLAB de Simulink:
      Fig 1 — Trayectoria XY completa (con inversión de ejes opcional)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # FIG 1 — Trayectoria XY completa con inversión opcional de ejes
    # ─────────────────────────────────────────────────────────────────────────
    fig1, ax = plt.subplots(figsize=(7.5, 7.5), facecolor='white')
    
    # Aplicar transformación de espejo a cada elemento
    ...
    # Líneas de formación (también transformadas)
    n_lin = 14
    ...
    for i in idxs:
        ...
        ax.plot([xi_t[0], xj_t[0]], [yi_t[0], yj_t[0]], '-',
                color=C_form, lw=0.9)
