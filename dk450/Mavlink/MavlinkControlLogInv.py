...
KP_YAW, KI_YAW, KD_YAW = 2.0, 0.0, 0.0

# ── Límites ───────────────────────────────────────────────────────────────────
INTEGRAL_LIMIT, INTEGRAL_YAW_LIMIT = 2.0, 1.0
V_MAX, YAW_RATE_MAX = 3.0, 1.0
RATE, DT = 20, 0.05
SPEED_FACTOR = 1.0
...
def _mavlink_reader(master, stop_event):
    while not stop_event.is_set():
        ...
        if msg is None or msg.get_srcSystem() != 1: continue
        with _state_lock:
...
def send_velocity_yawrate(master, vx, vy, vz_ned, yaw_rate):
    master.mav.set_position_target_local_ned_send(0, 1, 1,
...
def plot_results(buf):
    ...
    # Métricas
    def rms(v): return np.sqrt(np.mean(v**2)) if len(v) > 0 else 0
    print('\n============= MÉTRICAS =============')
    ...
    ax.plot(xdesS_t, ydesS_t, '--', color=C_refS, lw=LWS, label='Setpoint Seguidor', alpha=0.7)
    ...
    ax.set_title('Trayectoria XY con marcos de referencia y vector de formación', fontsize=12)
    ...
    # ==================== FIGURA 4 - Estados ====================
    fig4, axes = plt.subplots(4, 1, figsize=(10, 8), sharex=True, facecolor='white')
    ...
    # ==================== FIGURA 5 - Errores líder ====================
    fig5, axes = plt.subplots(4, 1, figsize=(10, 7), sharex=True, facecolor='white')
    fig5.suptitle('Errores del Líder respecto a su setpoint', fontsize=13, fontweight='bold')
    ...
    # ==================== FIGURA 6 - Errores seguidor ====================
    fig6, axes = plt.subplots(4, 1, figsize=(10, 7), sharex=True, facecolor='white')
    fig6.suptitle('Errores del Seguidor respecto a su setpoint', fontsize=13, fontweight='bold')
    ...
    # ==================== FIGURA 7 - Distancia ====================
    fig7, ax = plt.subplots(figsize=(10, 4), facecolor='white')
    ax.plot(t, dist, '-', color=C_dist, lw=LW, label='Distancia real L-S')
    ax.axhline(OFFSET_D, color=[0.0, 0.5, 0.2], ls='--', lw=1.5, label=f'd = {OFFSET_D:.1f} m')
    ax.set_xlabel('Tiempo [s]'); ax.set_ylabel('Distancia [m]')
    ax.set_title('Distancia entre Líder y Seguidor vs Tiempo', fontsize=12)
...
def main():
    ...
    if PRE_POSITION:
        ...
        input('\n▶️ Iniciar controlador de seguimiento? Presiona ENTER...\n')
    else:
        ...
        while True:
            ...
            if follower_ready(S):
                ...
                if int(t_log) % 5 == 0 and int(t_log - DT) % 5 != 0:
                    ...
                    print(f't={t_log:.1f}s | err_xy={err_xy:.3f}m')
