...
# ── Opciones de salida ─────────────────────────────────────────────────────────
SAVE_FIGURES = True    # True = guardar PNGs | False = mostrar en pantalla
...
def plot_results():
    ...
    # El setpoint Z del líder típicamente es constante (altitud de vuelo)
    # Lo estimamos como la mediana de la altitud del líder durante el vuelo estable
    if 'lz_sp' in df.columns:
        lz_sp = arr('lz_sp')
    else:
        # Si no existe la columna, asumimos que el setpoint es la altitud deseada
        # Podemos estimarla como la mediana de lz (asumiendo que ya está en régimen)
    ...
    # Obtener los valores deseados de formación desde los datos (primeros valores estables)
    ...
    # Calcular errores del líder correctamente
    ex_L = lxsp - lx
    ey_L = lysp - ly
    ez_L = lz_sp - lz
    ...
    print(f'  Muestras: {n}   Duración: {t[-1]-t[0]:.1f} s')
    print('='*55)
    ...
    # Dibujar el origen si está desplazado
    if ORIGEN_X != 0.0 or ORIGEN_Y != 0.0:
    ...
    # Dibujar marco de coordenadas en el origen
    ...
    # ── Fig 5b — Errores del líder respecto a su setpoint (AHORA CORRECTOS) ────
