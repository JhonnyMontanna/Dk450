...
# ⬇️ NUEVO: auto-modo vía MAVROS
try:
    from mavros_msgs.msg import GPSRAW  # ROS 2
    HAS_MAVROS_GPSRAW = True
except Exception:
    HAS_MAVROS_GPSRAW = False
...
class MultiDroneVisualizer(Node):
    def __init__(self):
        ...
        self.declare_parameter('calib_ang', 180.0)          # deg CCW
        self.declare_parameter('expected_local_x', 1.0)
        self.declare_parameter('expected_local_y', 1.0)

        # === Elipsoide: 3σ en XY + tamaño, y Z fijo por modo ===
        self.declare_parameter('auto_cov_mode', True)       # auto desde fix_type
        self.declare_parameter('cov_mode', 'fix')           # fallback: 'fix'|'float'|'gps'
        self.declare_parameter('confidence_k', 3.0)         # 3σ
        self.declare_parameter('include_drone_size', True)
        self.declare_parameter('drone_radius_xy', 0.55)     # m
        # Z fijo por modo (semieje vertical deseado)
        self.declare_parameter('lock_z_radius_per_mode', True)
        self.declare_parameter('z_radius_fix',   0.20)      # m
        self.declare_parameter('z_radius_float', 0.40)      # m
        self.declare_parameter('z_radius_gps',   0.60)      # m

        # σ por modo (para XY y también Z si NO bloqueas Z)
        ...
        self.h0   = self.get_parameter('origin_alt').value
        c_lat     = self.get_parameter('calib_lat').value
        c_lon     = self.get_parameter('calib_lon').value
        self.h_ref= self.get_parameter('calib_alt').value
        self.altitude_mode = self.get_parameter('altitude_mode').value
        self.calib_mode    = self.get_parameter('calib_mode').value
        self.calib_ang     = self.get_parameter('calib_ang').value
        self.expected_local_x = self.get_parameter('expected_local_x').value
        self.expected_local_y = self.get_parameter('expected_local_y').value
        ...
        self.lidar_z     = 0.0
        self.rel_alt     = 0.0
        self._cov_mode_param   = self.get_parameter('cov_mode').value
        self._cov_mode_runtime = self._cov_mode_param
        ...
        # Auto-modo desde MAVROS GPSRAW (dos rutas comunes, suscríbete a ambas por si acaso)
        if HAS_MAVROS_GPSRAW and self.get_parameter('auto_cov_mode').value:
            self.create_subscription(GPSRAW, f'/{self.ns}/gpsstatus/gps1/raw', self.cb_gpsraw, qos_be)
            self.create_subscription(GPSRAW, f'/{self.ns}/mavros/gpsstatus/gps1/raw', self.cb_gpsraw, qos_be)
    ...
    def cb_gpsraw(self, msg):
        ...
        if ft == 6:
            ...
            mode = 'float'
        elif ft in (3, 4, 8):  # 3D, DGPS, PPP => lo tratamos como "gps"
            mode = 'gps'
        else:
            mode = 'gps'
        self._cov_mode_runtime = mode
        self._last_fix_type = ft

    def _current_mode(self):
        if self.get_parameter('auto_cov_mode').value and self._cov_mode_runtime:
            return self._cov_mode_runtime
        return self._cov_mode_param

    # ---------------- Covarianza / Elipsoide ---------------- #
    def _sigmas_for_mode(self, mode):
        sx = self.get_parameter(f'sigma_{mode}_x').value
        sy = self.get_parameter(f'sigma_{mode}_y').value
        sz = self.get_parameter(f'sigma_{mode}_z').value
        return sx, sy, sz

    def _z_radius_for_mode(self, mode):
        if mode == 'fix':
            return self.get_parameter('z_radius_fix').value
        if mode == 'float':
            return self.get_parameter('z_radius_float').value
        return self.get_parameter('z_radius_gps').value

    def compute_axes_and_cov(self):
        """
        Devuelve (ax, ay, az, cov6x6) para RViz:
        - XY: sqrt( (k*σ)^2 + r_xy^2 ) si include_drone_size
        - Z:  si lock_z_radius_per_mode=True -> az = z_radius_<mode> (exacto)
              si False -> sqrt( (k*σz)^2 + r_z^2 )
        """
        mode = self._current_mode()
        k = self.get_parameter('confidence_k').value
        include_size = self.get_parameter('include_drone_size').value
        r_xy = self.get_parameter('drone_radius_xy').value
        lock_z = self.get_parameter('lock_z_radius_per_mode').value

        sx, sy, sz = self._sigmas_for_mode(mode)

        ax = math.sqrt((k*sx)**2 + (r_xy**2 if include_size else 0.0))
        ay = math.sqrt((k*sy)**2 + (r_xy**2 if include_size else 0.0))

        if lock_z:
            az = self._z_radius_for_mode(mode)
        else:
            r_z = self.get_parameter('drone_half_height_z').value
            az = math.sqrt((k*sz)**2 + (r_z**2 if include_size else 0.0))

        vroll  = self.get_parameter('var_roll').value
        vpitch = self.get_parameter('var_pitch').value
        vyaw   = self.get_parameter('var_yaw').value
    ...
    def cb_navsat(self, msg: NavSatFix):
        ...
        # Odometry + covarianza visible
        mode, ax, ay, az, cov = self.compute_axes_and_cov()
        ...
        if (now - self._last_debug_log).nanoseconds > 2_000_000_000:
            self.get_logger().info(
                f"[{self.ns}] mode={mode} fix_type={self._last_fix_type}  Var(x,y,z)=({cov[0]:.3f},{cov[7]:.3f},{cov[14]:.3f})  a=(%.3f,%.3f,%.3f)m"
                % (ax, ay, az)
            )
        ...
        # Marker elipsoide
        if self.get_parameter('show_ellipse_marker').value:
            ell = Marker()
